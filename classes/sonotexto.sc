...
SonoTexto {
	...
	*boot {
	...
	// the class method *rec allows to rec one, or all, the available Buffers when the argument is true
	// it is possible to warn about which synthdefs are available
	*rec { |b1 = 0, b2 = 0, b3 = 0, b4 = 0|
	...
	// the class method *write allows writing the temporary buffer sounds in the folder sonotexto placed in the Recordings Directory
	*write { |b1 = 0, b2 = 0, b3 = 0, b4 = 0|

		// this fork writes each Buffer in the hard disk when the argument is true. It waits for 60 milliseconds to avoid writing the same name to all Buffers.
		fork {
